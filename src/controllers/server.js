...
console.log('操作:',d)
    ...
    openWebSocket: function(){
        ...
        if('WebSocket' in window){
	        ...
	        _this.websocket.onopen = function() {
	        	console.info(locale().websocket.success ,' updateUrl:',_this.updateUrl);
	        	hideloading();
        ...
        method.createHookFunction('cooperativeMessage', data)
                ...
                const oldIndex = data?.data?.v?.index; // jxh start 0;//
                ...
                if (sheetToUpdate !== null) {
                  setTimeout(() => {
                    ...
                    const index = data.data.i; //oldIndex ;
					...
					if(flag) {
						Store.cooperativeEdit.changeCollaborationSize.forEach(val => {
							...
							//	console.log('item.range:',item.range)
								val.v = item.v[0] ||  (typeof item.v.range === 'string' ? JSON.parse(item.v.range) : item.v.range[0])  // item.range[0]
								val.i = index
							}
						})
					} else {
  ...
  console.log('type == 4进来：items:',items)
			  ...
			  // 自定义 获取当前sheet
				...
				//	  console.log('_this.websocket.send进来了')
				// 	 let msg = pako.gzip(encodeURIComponent(JSON.stringify( luckysheet.getAllSheets()[0].celldata)), {to: "string"});
				//   _this.websocket.send("sheetData:"+msg);
					//return
				//  }
	        }
    ...
    wsUpdateMsg: function(item) {
	    ...
	    if(type == "v"){ //单个单元格数据更新
	        ...
	        if(index == Store.currentSheetIndex){//更新数据为当前表格数据
				Store.flowdata = file.data;
			//	console.log('要更新的数据:',file.data,' item:',item)
				editor.webWorkerFlowDataCache(Store.flowdata);//worker存数据
