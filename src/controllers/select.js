...
function collaborativeEditBox() {
    ...
    Store.cooperativeEdit.changeCollaborationSize.forEach(value => {
        if (value.i == Store.currentSheetIndex) {
            ...
            //jxh start     $("#luckysheet-multipleRange-show-" + value.id).css({ "height": change_height, "width": change_width, "top": change_top + 'px', "left": change_left + 'px' })
            //jxh start      let change_bottom = $("#luckysheet-multipleRange-show-" + value.id)[0].offsetHeight - 1
            //jxh start      $("#luckysheet-multipleRange-show-" + value.id + ">.username").css({ "bottom": change_bottom + 'px' })
            // jxh start 注释上面三行 协同者边框乱了的问题
        }
    })
}
//复制选区虚线框
