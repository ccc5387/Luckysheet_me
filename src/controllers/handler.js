...
console.log('下拉11111 mousedown')
	...
	let ctrlVlastPasteTime = 0; // 上一次粘贴的时间
	const ctrlVpasteInterval = 2000; // 允许粘贴的最小时间间隔（毫秒）
    //粘贴事件处理
    $(document).on("paste.luckysheetEvent", function (e) {
        if (isEditMode()) {//此模式下禁用粘贴
            return;
        }
		// jxh start 阻止频繁粘贴,消息过多
		 const currentTime = Date.now();
		  if (currentTime - ctrlVlastPasteTime < ctrlVpasteInterval) {

			//event.preventDefault();
			alert('粘贴操作过于频繁，请稍后再试！');
			return;
		  } else {
			ctrlVlastPasteTime = currentTime;
		  }
		// jxh end 阻止频繁粘贴,消息过多
        ...
        if (selection.isPasteAction) {
            ...
            console.log('需要增加的行数:',inertRowNum)

            if(inertRowNum>0){ // "mc": cfg.merge
                // console.log('inertRowNum>0 进来了',inertRowNum)
