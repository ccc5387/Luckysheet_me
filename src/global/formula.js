...
console.log('442.4updatecell curv:',curv
 //   , 'callerLine:',callerLine
    ...
    getSheetData: function(sheetIndex) {
        ...
        return Store.luckysheetfile[getSheetIndex(sheetIndex)].data;
    },

     recalcAllFormulas: function(sheetIndex) {
    //const _this = window._luckysheet; // 假设全局 luckysheet 实例在 _luckysheet
        let _this = this;
    if (!_this) return;

    // 获取当前 sheet 数据
    const sheetData = _this.getSheetData(sheetIndex) || [];
    ...
    // 获取所有公式单元格
    const calcChains = _this.getAllFunctionGroup(sheetIndex);

    // 遍历公式，构建依赖图
    const formulaObjects = {};
    calcChains.forEach(cell => {
        const key = `r${cell.r}c${cell.c}i${cell.index}`;
        const formulaStr = getcellFormula(cell.r, cell.c, cell.index);
        if (!formulaStr) return;

        // 缓存解析结果
        let formulaArray = formulaCache[formulaStr];
        if (!formulaArray) {
            formulaArray = _this.getFormulaRanges(formulaStr, cell.index);
    ...
    // 执行计算
    uniqueRunList.forEach(f => {
        const result = _this.execfunction(f.formulaStr, f.r, f.c, f.index);
        // result[1] 是计算值，result[2] 是公式文本
        _this.execFunctionGlobalData[`${f.r}_${f.c}_${f.index}`] = {
            v: result[1],
            f: result[2]
        };
    });
    ...
    // 返回计算后的数据，可用于刷新 UI
    return _this.execFunctionGlobalData;
},

    /**
     * 解析公式字符串，返回引用的单元格区域
     * @param {string} formulaStr 公式字符串 (如 =SUM(A1:B2)+C3 )
     * @param {number} sheetIndex 当前 sheet 索引
     * @returns {Array<{sheetIndex:number, row:[number,number], column:[number,number]}>}
     */
     getFormulaRanges: function(formulaStr, sheetIndex) {
    ...
    const ranges = [];
    // 匹配单元格/区域，支持：A1, $A$1, A1:B5, Sheet2!A1, '工作表 2'!A1:B3
    const reg = /(?:'([^']+)'|([A-Za-z0-9_]+))?!?(\$?[A-Za-z]+\$?\d+)(:\$?[A-Za-z]+\$?\d+)?/g;

    let match;
    while ((match = reg.exec(formula)) !== null) {
        let refSheetName = match[1] || match[2]; // 表名
        let startRef = match[3]; // 起始单元格
        let endRef = match[4] ? match[4].slice(1) : null; // 结束单元格

        // 解析 sheetIndex
        let targetSheetIndex = sheetIndex;
        if (refSheetName) {
            const findSheet = this.getSheetByName?.(refSheetName);
            if (findSheet) targetSheetIndex = findSheet.index;
        }

        // 转换为行列坐标
        const [startRow, startCol] = this.a1ToRowCol(startRef);
        const [endRow, endCol] = endRef ? this.a1ToRowCol(endRef) : [startRow, startCol];

        ranges.push({
            sheetIndex: targetSheetIndex,
            row: [Math.min(startRow, endRow), Math.max(startRow, endRow)],
            column: [Math.min(startCol, endCol), Math.max(startCol, endCol)]
        });
    }
...
/**
 * A1 -> [row, col]
 * @param {string} ref 如 A1, $B$2
 * @returns {[number, number]} [rowIndex, colIndex] 0-based
 */
 a1ToRowCol: function (ref) {
    ref = ref.replace(/\$/g, ""); // 去掉 $
    const match = /^([A-Za-z]+)(\d+)$/.exec(ref);
    if (!match) return [0, 0];

    const colStr = match[1].toUpperCase();
    const row = parseInt(match[2], 10) - 1; // 行号从1开始
